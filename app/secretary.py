...
def move_doc_to_shelf(doc_number, shelf_number):
    ...
    print('Документ номер "{}" был перемещен на полку номер "{}"'.format(doc_number, shelf_number))
...
def add_new_doc(*args):
    ...
    append_doc_to_shelf(args[0], args[3])
    return args[3]
...
def secretary_program_start():
    ...
    while True:
        ...
        if user_command == 'p':
            ...
            new_doc_shelf_number = add_new_doc(new_doc_number, new_doc_type,
                                               new_doc_owner_name,
